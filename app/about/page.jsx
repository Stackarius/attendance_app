...
import { Twitter, Linkedin, Github,Lightbulb, User, Shield, Smile } from 'lucide-react'
import { motion } from "framer-motion";

export default function AboutUs() {
  const team = [
    {
      name: "Agbeniga Khadijat Okikiola",
      matric: "CS20200203731",
    ...
    {
      name: "Fasanu Jumoke Damilola",
      matric: "CS20230206770",
      department: "Computer Science",
    ...
    {
      name: "Adeleke Adbulbasheet Ayomide",
      matric: "CS20230205800",
      department: "Computer Science",
      bio: "Manages finances and helps maintain sustainable growth.",
      img: "/male.jpg",
      twitter: "#",
  ...
  return (
    <div className="min-h-screen bg-gray-50 relative overflow-hidden">
      ...
      >
        <motion.div
          className="relative w-72 h-72 md:w-120 md:h-120 mb-8 md:mb-0"
      ...
      >
        <motion.div variants={fadeUpVariant} className="bg-white shadow-lg rounded-lg p-8 flex flex-col items-center text-center hover:scale-105 transition-transform">
          <Lightbulb size={48} className="text-indigo-600 mb-4" />
        ...
        <motion.div variants={fadeUpVariant} className="bg-white shadow-lg rounded-lg p-8 flex flex-col items-center text-center hover:scale-105 transition-transform">
          <User size={48} className="text-indigo-600 mb-4" />
      ...
      >
        ...
        <div className="max-w-6xl mx-auto items-center grid sm:grid-cols-3 md:grid-cols-3 gap-8">
          {team.map((member, idx) => (
            <motion.div key={idx} variants={fadeUpVariant} className="bg-white shadow-lg rounded-lg p-6 text-center hover:scale-105 transition-transform relative group overflow-hidden">
              ...
              <div className="flex justify-center gap-4 text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity absolute bottom-4 left-0 w-full">
                {member.twitter && <a href={member.twitter} target="_blank" rel="noopener noreferrer"><Twitter size={24} /></a>}
                {member.linkedin && <a href={member.linkedin} target="_blank" rel="noopener noreferrer"><Linkedin size={24} /></a>}
                {member.github && <a href={member.github} target="_blank" rel="noopener noreferrer"><Github size={24} /></a>}
              </div>  
            </motion.div>
          ))}
        </div>
